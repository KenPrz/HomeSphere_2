<template>
    <Head title="Appliances" />
    <AuthenticatedLayout>
      <template #header>
        <h2 class="font-semibold text-xl text-gray-800 leading-tight">
          Appliances
        </h2>
      </template>
      <main>
        <div class="container">
            <h1 class="mx-10 py-3 text-3xl font-bold">
            List of Appliances
          </h1>
          <div class="flex-grow mx-10 mt-3 p-3 rounded-lg bg-white">
            <div class="flex items-center border border-slate-500 py-1 sm:w-1/2 md:w-2/3 lg:w-1/2 xl:w-1/3 h-12 rounded-full text-1xl text-left">
              <div class="w-full pe-5 flex">
                <div class="ml-5 self-center">
                  <img src="img-assets/vectors/search.svg" alt="search" />
                </div>
                <input
                  v-model="searchInput"
                  id="search"
                  name="searchInput"
                  placeholder="Search"
                  class="ms-5 w-full border-0 focus:ring-0"
                />
              </div>
            </div>
          </div>
          <div class="mx-10 mt-5">
            <Table
              :tableHeaders="tableHeaders"
              :tableData="filteredTableData"
              :maxHeight="maxHeight"
              :itemsPerPage="6"
              :Pagenated="true"
            />
            <div v-if="filteredTableData.length === 0" class="text-center mt-4 text-red-500">
            No Data Found
          </div>
          </div>
        </div>
      </main>
    </AuthenticatedLayout>
</template>

<script setup>
import { ref, computed } from "vue";
import Table from "@/Components/Table.vue";
import AuthenticatedLayout from "@/Layouts/AuthenticatedLayout.vue";
import { Head } from "@inertiajs/vue3";

const { appliances } = defineProps(["appliances"]);

const searchInput = ref(""); // Define the searchInput variable
const tableData = ref(appliances); 

const filteredTableData = computed(() => {
  const searchTerm = searchInput.value.trim().toLowerCase();

  if (!searchTerm) {
    return tableData.value;
  }

  return tableData.value.filter((item) => {
    const itemName = item.Room.toLowerCase() + item.Type.toLowerCase();

    return itemName.includes(searchTerm);
  });
});
</script>

<script>
export default {
    components: {
        Table,
    },
    props: {
    filters: Object,
    appliances: Object,
    },
    data() {
        return {
            tableHeaders: [
                {
                    text: "Room",
                },
                {
                    text: "Type",
                },
                {
                    text: "Name",
                },
                {
                    text: "Status",
                },
            ],
            tableData: this.$page.props.appliances,
            maxHeight: "max-h-80",
        };
    },
};
</script>