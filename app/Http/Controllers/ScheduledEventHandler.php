...
class ScheduledEventHandler extends Controller
{
    ...
    private function handleDaily($scheduledMode){
        $is_mode_active = DB::table('modes')->where('id', $scheduledMode->mode_id)->value('is_active');
        if($is_mode_active==false){
            DB::table('modes')->where('id', $scheduledMode->id)
            ->update([
                'is_active' => true,
            ]);
            $data = DB::table('mode_devices')
                ->where('mode_id', $scheduledMode->mode_id)
                ->get();
